...
def update_figure(game_id, play_id):
    ...
    fig_dict["layout"]["xaxis2"] = {
        "range": [0, play_df["frameId"].max()],
        "title": "frame",
    }
    fig_dict["layout"]["xaxis3"] = {
        "range": [0, play_df["frameId"].max()],
        "title": "frame",
    }
    # fig_dict["layout"]["yaxis2"] = {
    #     "range": None,  # TODO
    #     "title": "sum QB trace hits",
    #     "domain": [0.55, 1],
    # }
    ...
    # compute metric
    # # compute metric | qb hits
    qb_radius = 0.5
    oline_radius = 0.3
    phi_max = 30
    phi_num = 140
    play_df.loc[:, "o_map"] = utils.map_angle(play_df.loc[:, "o"])
    play_df.loc[:, "dir_map"] = utils.map_angle(play_df.loc[:, "dir"])
    frames_max = play_df[
        (play_df["gameId"] == game_id) & (play_df["playId"] == play_id)
    ]["frameId"].max()
    light_hits = []
    ...
    fig_dict["layout"]["yaxis2"] = {
        "range": [0, qb_hit_series.max()],  # TODO
        "title": "sum QB trace hits",
        "domain": [0.33, 0.63],
    }

    # # compute metric | oline blocks
    light_blocked_player = light_hits[:, :, :-1].sum(axis=1)
    light_blocked_total = light_blocked_player.sum(axis=1)

    fig_dict["layout"]["yaxis3"] = {
        "range": [0, light_blocked_total.max()],  # TODO
        "title": "OLine trace blocks",
        "domain": [0.66, 1.00],
    }

    # make initial frame
    # # make initial frame play itself
    intital_frame_id = 1
    ...
    # # make intitial frame qb hit metric
    metric_trace = {
        "x": frame_ids,
        "y": qb_hit_series,
        "mode": "lines",
    ...
    fig_dict["data"].append(metric_trace)
    ...
    # # make initial frame oline blocks
    metric_trace = {
        "x": frame_ids,
        "y": light_blocked_total,
        "mode": "lines",
        "xaxis": "x3",
        "yaxis": "y3",
    }
    ...
    for oplayer_col_ind in range(light_blocked_player.shape[1]):
        oline_series = light_blocked_player[:, oplayer_col_ind]
        metric_trace = {
            "x": frame_ids,
            "y": oline_series,
            "mode": "lines",
    ...
    for frame_id in frame_ids:
        ...
        # # make frame qb hit metric
        data_dict = {
            "x": [frame_id],
            "y": [qb_hit_series[frame_id - 1]],
            "mode": "markers+lines",
            "xaxis": "x2",
            "yaxis": "y2",
        }
        frame["data"].append(data_dict)
        # frame["data"].append(metric_trace)

        # # make frame oline block metric
        data_dict = {
            "x": [frame_id],
            "y": [light_blocked_total[frame_id - 1]],
            "mode": "markers+lines",
            "xaxis": "x3",
            "yaxis": "y3",
        }
